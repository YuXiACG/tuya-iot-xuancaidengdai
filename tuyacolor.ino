...
int maxColor = sizeof(usualColor)/sizeof(usualColor[0]) - 1;
...
uint8_t ZiDingYi_mode=0;
long DengZu=0;
long QieHuan_Time=0;
long GaiBian_Time=0;
uint8_t ShiZhi_mode=0;
u32 color[20];
...
unsigned char pid[] = {"yaxbeooas3ecmnoa"};//*********处替换成涂鸦IoT平台自己创建的产品的PID
...
unsigned char dp_process(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  switch(dpid) 
  {
    ...
    case DPID_DREAMLIGHT_SCENE_MODE: //炫彩情景
      ...
      if(scene_mode>=200)
      {
        DengZu=(length-9)/3;//颜色单元组数量
        ZiDingYi_mode=value[2];
        QieHuan_Time=value[3];
        GaiBian_Time=value[4];
        ShiZhi_mode=value[5];
        for(int i=0;i<DengZu;i++)
        {
          Hue=(value[9+i*3]<<8)+value[10+i*3];
          Hue=Hue*182;
          Sat=value[11+i*3]*2.55;
          Val=value[8]*2.55;
          color[i] = strip.gamma32(strip.ColorHSV(Hue,Sat,Val)); // hue -> RGB
        }
        strip.fill(strip.Color(0,0,0),0,30);
        switch(ZiDingYi_mode)
        {
          case 0://静态
            for(int i=0;i<DengZu;i++)
            {
              color_jingtai(color[i],PIXEL_COUNT,QieHuan_Time);
            }
            strip.fill(strip.Color(0,0,0),0,30);
            break;
          case 1://渐变
          ...
          case 4://闪烁
            break;
          case 10://流水
            if((ShiZhi_mode & 16)==0)//60,C0,E0-顺时针
            {
              for(int i=0;i<DengZu;i++)
                color_liushui(color[i],30,GaiBian_Time,GaiBian_Time/20,0);
            }
            else if((ShiZhi_mode & 16)==16)//70,D0,F0-逆时针
            {
              for(int i=0;i<DengZu;i++)
                color_liushui(color[i],30,GaiBian_Time,GaiBian_Time/20,1);
            }
            break;
          case 11://彩虹
            break;
      ...
      else
      {
        switch(scene_mode)
        {
         ...
         case 1:
           switch(i)
           {
              case 0: colorWipe(strip.Color(255,   0,   0), 50);    // Red
                      i=1;break;
              case 1: colorWipe(strip.Color(0,   0,   255), 50);    // Red
                      i=0;break;
           }
           break;
         case 2:
           ...
           delay(10);
           colorWipe(strip.Color(  78, 24,   146), 50);    // Green
           delay(10);
           break;
         case 3:
           flowLight();
         ...
         case 4:
           ...
           break;
         case 5:
           theaterChase(strip.Color(  0,   255, 0), 50); // 绿
           break;
         case 6:
           theaterChase(strip.Color(  0,   0, 127), 50); // 蓝
           delay(20);
         ...
         case 9:
           for(int i=0;i<5;i++)
           {
             colorWipe2(strip.Color(255,  0,  0),strip.Color(255, 97,  0),PIXEL_COUNT, 50); 
             delay(20);
             colorWipe3(strip.Color(  0,255,  0),strip.Color(  0,255,255),PIXEL_COUNT, 50); 
             delay(20);
             colorWipe2(strip.Color(  0,  0,255),strip.Color(160, 32,240),PIXEL_COUNT, 50); 
             delay(20);
             colorWipe3(strip.Color(255,255,  0),strip.Color(  0,110, 95),PIXEL_COUNT, 50); 
             delay(20);
           }
           break;        
        }
...
void color_jingtai(uint32_t color,int a,int wait)//静态
{
  for(int i=0;i<a;i++)
  {
...
void color_liushui(uint32_t color,int a,int wait,int b,uint8_t mode1)//流水灯
{
  switch(mode1)
  {
    case 0:
      for(int i=0;i<a+b;i++)
      {
        ...
        strip.show();
        delay(wait); 
        strip.setPixelColor(i-b, strip.Color(0,0,0)); 
        strip.show();
      }
      break;
    case 1:
      for(int i=a; i>0-b-1; i--)    // For each pixel in strip...
      {
        strip.setPixelColor(i, color); 
        strip.show();
        delay(wait); 
...
void colorWipe(uint32_t color, int wait)//灯正全亮, RGB颜色,延时
{
...
void colorWipe1(uint32_t color1,int a,int wait)//灯倒全亮, RGB颜色,延时
{
  for(int i=a; i>-1; i--)    // For each pixel in strip...
  {
...
void colorWipe2(uint32_t color1,uint32_t color2,int a,int wait)//灯两边向中间全亮, RGB颜色,延时
{
  for(int i=a; i>a/2-1; i--)    // For each pixel in strip...
  {
    strip.setPixelColor(i, color1);         //  Set pixel's color (in RAM)
    strip.setPixelColor(a-i-1, color2);         //  Set pixel's color (in RAM)
    strip.show();                          //  Update strip to match
    delay(wait);                           //  Pause for a moment
  }
}
void colorWipe3(uint32_t color1,uint32_t color2,int a,int wait)//灯两边向中间全亮, RGB颜色,延时
{
  for(int i=0; i<a/2+1; i++)    // For each pixel in strip...
  {
    strip.setPixelColor(a/2-i, color1);         //  Set pixel's color (in RAM)
    strip.setPixelColor(a/2+i, color2);         //  Set pixel's color (in RAM)
    strip.show();                          //  Update strip to match
    delay(wait);                           //  Pause for a moment
  }
}
...
// between frames.
void theaterChase(uint32_t color, int wait) //追逐灯,RGB颜色,延迟帧时间
{
  for(int a=0; a<10; a++) 
  {  // Repeat 10 times...
    for(int b=0; b<3; b++) 
...
void theaterChaseRainbow(int wait) //彩虹追逐灯,延迟帧时间
{
...
void flowLight()
{
  ...
  {
     flowLightBasic(usualColor[i].r,usualColor[i].g,usualColor[i].b);
  }
}

void flowLightRandomColor()
{
  flowLightBasic(random(0,255),random(0,255),random(0,255));
}

bool flowLightBasic(int r,int g,int b)
{
  int i;
